...
if __name__ == "__main__":
    ...
    args = p.parse_args()
    query = session.query(Questions).filter(Questions.id==args.question)
    for q in query:
        # print "Question %d %s: %d answers" % (q.id, q.question, len(q.tasks_collection))
        print "question answer given about utterance"
...
#            print "  task %d, recording %d, theme %d" % (t.id, nonint(t.question_recording_id), t.theme_id)
            if t.question_recording_id:
                ...
                for a, r in session.query(Answers, Recordings).filter(Answers.id==t.answer_id, Recordings.id==t.question_recording_id):
                    ...
                    text = session.query(Texts, TaskText, Tasks).filter(Texts.id==TaskText.text_id, TaskText.task_id==Tasks.id, Tasks.recording_id==r.id).first()
                    if text:
                        text = text[0]
                        print '%d  %d %d %d "%s"' % (q.id, nonint(a.answer_numeric), t.profile_id, r.profile_id, text.text)
            else:
                ...
                for a in session.query(Answers).filter(Answers.id==t.answer_id):
                    if q.component_id == 1: ## slider
                        ...
                        loc = session.query(AnswerLocation, Locations).filter(AnswerLocation.answer_id==a.id, AnswerLocation.location_id==Locations.id).first()
                        if loc:
                            loc = loc[1]
                            print "    answer %d value (%8.6f, %8.6f, %d) given by %d" % (a.id, loc.latitude, loc.longitude, loc.mapzoom, t.profile_id)
                    else:
                        ao = session.query(AnswerOption, Options).filter(AnswerOption.answer_id==a.id, AnswerOption.option_id==Options.id).first()
                        if ao:
                            ao = ao[1]
